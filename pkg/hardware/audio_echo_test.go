...
func TestAudioManager_EchoSuppression(t *testing.T) {
	...
	// 测试3: 高能量音频在TTS播放期间可能通过
	highEnergyData := make([]byte, 1024)
	for i := range highEnergyData {
		highEnergyData[i] = 255 // 最大值，产生高能量
	}

	processedData, shouldProcess = manager.ProcessInputAudio(highEnergyData, true)
	// 这个测试可能通过也可能不通过，取决于具体的阈值设置
	t.Logf("高能量音频在TTS播放期间是否通过: %v", shouldProcess)
}
