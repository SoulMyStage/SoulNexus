...
import (
	...
	"github.com/code-100-precent/LingEcho/pkg/hardware1"
	hardwarefinal "github.com/code-100-precent/LingEcho/pkg/hardwarefinal/handler"
...
// HandleHardwareWebSocketVoice 处理硬件WebSocket语音连接（与xiaozhi-esp32兼容）
// 从Header中获取Device-Id（MAC地址），查询设备绑定的助手，动态获取配置
func (h *Handlers) HandleHardwareWebSocketVoice1(c *gin.Context) {
	// 从Header获取Device-Id（MAC地址），与xiaozhi-esp32兼容
	deviceID := c.GetHeader("Device-Id")
	if deviceID == "" {
		// 如果Header中没有，尝试从URL查询参数获取（xiaozhi-esp32兼容）
	...
	// 创建WebSocket处理器
	handler := hardwarefinal.NewHardwareHandler(h.db, logger.Lg)
